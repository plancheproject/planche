Ext.define('Planche.dbms.mysql', function() {
    ...
    var queries = {
        ...
        DROP_PROCEDURE          : 'DROP PROCEDURE `{0}`.`{1}`',
        DROP_FUNCTION           : 'DROP FUNCTION `{0}`.`{1}`',
        DROP_TRIGGER            : 'DROP TRIGGER `{0}`.`{1}`',
        DROP_EVENT              : 'DROP EVENT `{0}`.`{1}`',
        DROP_VIEW               : 'DROP VIEW `{0}`.`{1}`',
