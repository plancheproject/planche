Ext.define('Planche.dbms.mysql', function() {
    ...
    var queries = {
        ...
        CREATE_FUNCTION         : 'DELIMITER $$ CREATE /*[DEFINER = { user | CURRENT_USER }]*/ FUNCTION `{0}`.`{1}`() RETURNS INT /*{STRING|INTEGER|REAL|DECIMAL}*/ /*LANGUAGE SQL | [NOT] DETERMINISTIC | { CONTAINS SQL | NO SQL | READS SQL DATA | MODIFIES SQL DATA } | SQL SECURITY { DEFINER | INVOKER } | COMMENT \'string\'*/ BEGIN END$$ DELIMITER ;',
        CREATE_TRIGGER          : 'DELIMITER $$ CREATE /*[DEFINER = { user | CURRENT_USER }]*/ TRIGGER `{0}`.`{1}` BEFORE /* or AFTER */ INSERT /* or UPDATE or DELETE */ ON `{0}`.`<Table Name>` FOR EACH ROW BEGIN END$$ DELIMITER ;',
        CREATE_EVENT            : 'DELIMITER $$ -- SET GLOBAL event_scheduler = ON$$\n-- required for event to execute but not create\nCREATE /*[DEFINER = { user | CURRENT_USER }]*/	EVENT `{0}`.`{1}` ON SCHEDULE EVERY 1 HOUR /* uncomment the example below you want to use */\n-- scheduleexample 1: run once\n \t--  AT \'YYYY-MM-DD HH:MM.SS\'/CURRENT_TIMESTAMP { + INTERVAL 1 [HOUR|MONTH|WEEK|DAY|MINUTE|...] }\n -- scheduleexample 2: run at intervals forever after creation\n \t-- EVERY 1 [HOUR|MONTH|WEEK|DAY|MINUTE|...]\n -- scheduleexample 3: specified start time, end time and interval for execution\n /*EVERY 1  [HOUR|MONTH|WEEK|DAY|MINUTE|...]\n STARTS CURRENT_TIMESTAMP/\'YYYY-MM-DD HH:MM.SS\' { + INTERVAL 1[HOUR|MONTH|WEEK|DAY|MINUTE|...] }\n ENDS CURRENT_TIMESTAMP/\'YYYY-MM-DD HH:MM.SS\' { + INTERVAL 1 [HOUR|MONTH|WEEK|DAY|MINUTE|...] } */ /*[ON COMPLETION [NOT] PRESERVE] [ENABLE | DISABLE] [COMMENT \'comment\']*/ DO BEGIN /* SQL Statements */ END$$ DELIMITER ;',
