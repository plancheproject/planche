Ext.define('Planche.lib.QueryParser', {
	constructor : function(engine){
		...
		this.type = Planche.lib.QueryTokenType;
		...
		this.regexpBoundaries		= new RegExp('^('+this.addSlashes(this.boundaries).join("|")+')', "g");
		this.regexpSplit			= new RegExp('('+this.addSlashes(this.boundaries).join("|")+'|\\s)', "g");
		this.regexpDelimiter		= /^;/g;
		this.regexpFunctions 		= new RegExp('^('+this.engine.getFunctions().join("|")+')', "gi");
		this.regexpKeywords 		= new RegExp('^('+this.engine.getKeywords().join("|")+')', "gi");
	...
	getNextToken : function(string){
		...
		if(matches = string.match(this.regexpKeywords)){

			token.type = this.type.KEYWORD;
			token.value = matches[0];
			return token;
		}
	...
	addSlashes : function(arr){
		...
		if(typeof arr == 'string'){
			...
			return Ext.Array.map(arr.split(''), function(item, idx){ return "\\"+item }).join('');
		}
		else {

			return Ext.Array.map(arr, function(item, idx){ return "\\"+item });
		}
	},
