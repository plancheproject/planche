Ext.define('Planche.lib.QueryParser', {
	constructor : function(engine){
		...
		this.regexpReservedWords	= new RegExp('^('+this.engine.getReservedWords().join("|")+')($|\s|'+boundaries+')', "gi");
	...
	getNextToken : function(string){
		...
		if(matches = string.match(this.regexpReservedWords)){
            ...
            var upper = matches[0].toUpperCase();

            if(['FORM', 'UPDATE', 'INSERT'].indexOf(upper) > -1){
